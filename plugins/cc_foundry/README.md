...
Reads JSONL logs foundry `task-log.js` hook already writes for the clock side (`bin/timing_analyzer.py`) and Claude Code transcripts for the cost side (`bin/cost_analyzer.py`) — answers "where did wall clock go in `/oss:resolve`?" and "what did this session cost?" alike. Agents run in the background, so a spawn-only row carries a false-zero `duration_ms`; the real span is recovered by joining the matching `started→completed` pair in `invocations.jsonl`. The cost side is best-effort: if no transcripts fall in the window, the `## Tokens & cost` section is omitted and the clock report still ships.
...
Blocks only audit's fix follow-up until aggregate.md and valid summary.jsonl exist and Step 7 delivers Audit Report, exact Total, and every finding. A valid empty summary permits a zero-finding audit. Recovery questions remain available. Finish the producer, print the required current output, then retry the workflow follow-up. Existing sentinel expiry remains unchanged; it is not a completion receipt.
...
└── hooks/
    ├── hooks.json               hook registrations (${CLAUDE_PLUGIN_ROOT} paths); the two SessionEnd entries (task-log.js, audit-close.js) carry an explicit 15 s timeout — the cap every plugin's SessionEnd audit-close.js registration shares
    ...
    ├── enforce-audit-header.js  Blocks only audit's fix follow-up until aggregate.md and valid summary.jsonl exist and Step 7 delivers Audit Report, exact Total, and every finding. A valid empty summary permits a zero-finding audit. Recovery questions remain available.
    ├── enforce-profile-header.js Blocks only the owning workflow follow-up until its report and matching current-turn header table are verified. Diagnostic/recovery questions remain available; unreadable delivery evidence is not success.
    └── report-header-table.js  shared local workflow-recognition and current-report delivery checks
```
...
| `check_bin_test_coverage.py`   | Require every bin/ script to have a real test file (audit Check R4).                                                                                                                                                                                                                          |
...
| `check_install_state.py`       | Validate post-install state under `~/.claude` for `/foundry:audit`, including native Windows symlink targets.                                                                                                                                                                                 |
...
| `check_plugin_version_sync.py` | Require matching dual-host manifests, retain tracked host manifests in shipped plugins, and check changed shipped JSON version, schema, and protocol fields plus matching Python dictionary literals and module-level `*_VERSION`, `_SCHEMA`, and `PROTOCOL` integers against committed HEAD. |
...
| `check_skill_contracts.py`     | Run structural contract checks over skill Markdown for `/foundry:audit`.                                                                                                                                                                                                                      |
...
| `scan_efficiency_signals.py`   | Emit deterministic cost signals for `/foundry:audit --efficiency`.                                                                                                                                                                                                                            |

> Plugin manifests accept numeric `X.Y.Z` versions: unchanged or one patch, minor, or major bump from committed HEAD. Python dictionary literals bind to an enclosing class/function and a direct named assignment when present, then compare by version field name. A newly inserted named assignment does not shift another producer's baseline. Changed duplicate inline fields without stable assignment names fail closed because the checker cannot identify which producer changed. Computed keys, dynamic values, and reassignment through a dictionary variable are outside this static check.
...
| ----------------------------------- | --------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `test_task_log_js.py`               | `task-log.js`               | 34    | Agent lifecycle (Pre/PostToolUse, SubagentStart/Stop), tool_use_id→agent_id re-key on SubagentStart (direct + agent_type-fallback match, including named agents matched by name) so SubagentStop's unlink actually lands, background agents kept past dispatch-time PostToolUse, pending/ race condition, worktree `last_active` refresh on tool activity, codex skill tracking, per-tool counters (`last` stamp per call, `inflight` opened at PreToolUse and released on PostToolUse/Failure, floored at 0 on duplicate fire)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `test_statusline_js.py`             | `statusline.js`             | 23    | `🤖` segment: empty/active/stale agents, non-worktree agent stays visible past 10 min (30-min backstop) and drops past 30 min, long-running agent kept visible by fresh `last_active`, codex agent type label (via `agents/` and the `codex/` dir), missing-`agents/`-dir tolerance. `🛠️` segment: freshness from `last` (a call 5 s into a 40 s-old window stays shown), in-flight call visible past the 30 s window, finished call and leaked in-flight slot dropped, legacy `since`-only record still renders                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `test_enforce_audit_header_js.py`   | `enforce-audit-header.js`   | —     | Current aggregate delivery, zero findings, missing/unreadable evidence, recovery questions, and sentinel lifecycle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `test_enforce_profile_header_js.py` | `enforce-profile-header.js` | —     | Current header delivery, recovery boundary, state-file parsing, and sentinel lifecycle.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `test_report_header_table_js.py`    | `report-header-table.js`    | —     | Matching-table delivery, unrelated tables, workflow recognition, current-turn parent transcript extraction, and retained legacy formatting helpers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `test_resolve_shared_path.py`      | `resolve_shared_path.py`                    | 23    | Plugin/subdir validation, path traversal rejection, tier-1/2/3 resolution cascade, registry record beating a newer cache dir, un-prefixed provider source fallback                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `test_audit_preflight.py`          | `audit_preflight.py`                        | 18    | Bare-scope/challenge-alias parsing, `--keep` flag extraction, tool-presence marker caching (fresh/expired/corrupt), session-scoped state files, mutually-exclusive-flag and missing-dir exits, template resolution                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `test_check_bin_test_coverage.py`  | `check_bin_test_coverage.py`                | 17    | Real-function counting vs stub bodies (pass/ellipsis), async and nested-class tests counted, non-test functions ignored, syntax-error files treated as covered, missing/empty/stub-only test files flagged, private scripts skipped, `--local` clean/dirty tree gating                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `test_check_install_state.py`      | `check_install_state.py`                    | 20    | Nested/flat registry shape resolution, missing registry/foundry/cache-dir severity, settings merge completeness, statusline and allow-list checks, bridge enablement, stale-hook and rule-link staleness, native Windows and POSIX symlink-target checks, `--check` dispatch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `test_check_plugin_layout.py`      | `check_plugin_layout.py`                    | 26    | Manifest presence/validity/required-fields/name checks, real dir/file existence severities, reference resolution, hooks.json presence and validity, hooks-registry sync across plugin/settings, setup-skill keyword coverage, absent-CLI and `claude plugin validate` timeout degrading to a skip so 8f/8g still run, absent-plugin-dir and broken-plugin runs both reporting on stdout at exit 0, plugin-root reference extraction                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `test_check_rtk_alignment.py`      | `check_rtk_alignment.py`                    | 17    | Single/double-quoted prefix-array parsing, missing/unreadable file fallback, indented-command extraction, aligned-list pass, unknown/unlisted/meta-command classification, `grep -w` word-boundary semantics (prose-only mention valid, hyphen a boundary, bare substring still invalid), RTK/hook-absent skip, unparsable list skip, `rtk --help` timeout skip, misaligned prefixes reported on stdout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `test_check_skill_contracts.py`    | `check_skill_contracts.py`                  | 27    | Bare-comment and shell-timeout exemption detection, single/multiline Bash-call timeout presence, `add_argument` exemption that a docstring mention does not satisfy, config-file discovery across both roots (installed `.claude/agents/*.md`, source `plugins/*/agents/*.md`, nested `rules/_full/`, unrelated Markdown excluded), mode-file shadow and overlap detection, `--check` dispatch, non-local 32f skip                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `test_list_audit_files.py`         | `list_audit_files.py`                       | 14    | Local sweep finds skill entrypoints/modes/templates/shared, excludes nested agent sidecars and `rules/_full`, installed sweep narrower, dedupe+sort, block counting per file, header/relative-path labelling, local vs installed root selection                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `test_measure_config_size.py`      | `measure_config_size.py`                    | 14    | Empty-dir header-only output, agent/skill/rules size thresholds (warn/fail), skill path labelling, tighter rules budget, global CLAUDE.md counted once, `--mode` required and inventory/overhead dispatch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
