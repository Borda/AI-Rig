...
**Codemap-first protocol**: (1) **Skill-first** — consult the query output above before any Grep/Glob/Read aimed at imports, callers, or symbol contracts for something already listed there — applies to code-implementation tasks (reproducing a paper's method in an existing codebase), not paper/literature analysis, which has no codebase target. (2) **Bounded call budget** — symbol not covered above → up to 3 additional `codemap-py query` calls this task. (3) **Hard stop on `query_complete: true`** (legacy `exhaustive: true` only when `query_complete` is absent) — a result passing the reuse gate is final for its direction, no follow-up Grep/Read/query to re-confirm it. `codemap-py` not found or index missing: block above produces no output — proceed with normal file-read behaviour, no protocol applies.
...
- **Surfacing low-severity observations as findings**: items below medium severity (e.g., missing secondary citations for well-known techniques, uncited common-knowledge augmentations) should be observations, not findings, when analysis targets attribution accuracy or contribution validity. Flag under separate "Minor Observations" heading at end, separated from core findings. Prevents low-severity noise inflating finding count, diluting precision. An observation correctly placed under "Minor Observations" rather than "Findings" does not count as a false positive in calibration scoring — only items in the main Findings section lacking ground-truth support are FPs. Separating Findings from Minor Observations is the correct pattern.
