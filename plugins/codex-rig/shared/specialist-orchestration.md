...
For every workflow, inspect the actual launcher's child-control contract before preparing a spawned pass. Separate requested role settings, inherited parent settings, supported child controls, and observed effective controls. A missing override argument alone does not rule out a documented enforced inheritance route, but parent settings alone do not prove that route. A workspace-write parent plus no verified child override cannot establish read-only children. Never manufacture host declarations or claim that an instruction to avoid edits enforces filesystem isolation.

The promoted `code-review`, `implement`, and `manage` consumers share preflight admission: transcribe supported child controls into frozen `read_host={"source":"runtime-tool-contract","sandbox_mode":"read-only","approval_policy":"never"}` only when the actual launcher contract establishes them. Historical `review_host` is accepted for code-review only. This declaration admits planning; it is not host observation or runtime proof. Missing, requested-only, or incompatible controls make `auto` select `serial` with `fallback_reason=host-controls-unavailable-before-dispatch`; explicit `parallel-read` stops before dispatch. Keep mandatory profile and independence requirements: a serial resolution never satisfies BROAD/HIGH_RISK review's independent QA/challenger gate. Other workflows apply the same mandatory/preferred route classification before dispatch, even when they do not use the promoted consumer helper.
