...
## GitHub Workflow Consent

`--approve-gh` is optional invocation syntax, not a conversation reply protocol. Never ask the user to reply with `--approve-gh`, repeat the skill invocation, or add the flag to continue. Existing unambiguous authorization for the same scope is sufficient without the flag; absence of the flag is not evidence that consent is missing. Do not set `approve_gh=true` from an ordinary answer: record the scoped workflow consent separately and retain the flag's direct-invocation parsing boundary.

Before required GitHub collection, reuse scoped consent already supplied. Only if consent is genuinely missing, apply [User Questions](codex-user-questions.md): ask through a permitted native control with `Approve` / `Deny` action labels, bind the answer to the target and effects, and keep collection pending. If the host requires plain text or neither control is suitable, explain that restriction and ask about the action in ordinary language; do not demand an invocation flag. A valid answer resumes the pending workflow without a rerun. Preserve any denial and its stop rules.

After either form of scoped consent, preserve the direct helper command and matching host-rule boundaries in [PR Collection Preapproval](#pr-collection-preapproval) for `collect_pr.py` or [GitHub Reader Preapproval](#github-reader-preapproval) for `github_read.py`, together with [Managed Host Preapproval](#managed-host-preapproval). Do not wrap either helper in `rtk` or change its argv merely because consent arrived without the flag. Local-only work does not enter this approval path.

Workflow consent never supplies runtime permission. Execute the owning command through the dedicated runtime permission mechanism under the existing network, credential, checkout, and denial boundaries; no conversational answer creates saved rules or permits broader work.
...
1. Bind the target before suggesting persistence. Validate a URL target as a canonical GitHub PR URL (`https://github.com/<owner>/<repository>/pull/<positive-number>`) using the collector's existing target and remote identity rules. Before the first `collect_pr.py` invocation with numeric user input, run local `select-git-remote.py --canonical-pr-url <positive-number>` to resolve only from one unambiguous local GitHub repository; replace a numeric user target with that locally bound canonical URL in the actual `--target` argument. Canonical repository URL is mandatory to prevent a numeric PR cross-repository leak. If numeric input has no unique local GitHub repository, ask for a canonical PR URL instead of guessing or launching a numeric collector command. For current-branch/no explicit PR, use one-shot collection without prefix through normal runtime approval; only successful authoritative `pr.json` may supply identity for subsequent collection. Never rerun successful collection merely to save permission.
2. Construct the direct collector command with this literal argv prefix: actual Python executable, absolute installed `collect_pr.py` path, `--target`, canonical GitHub PR URL. The command and `prefix_rule` must contain the same URL; pass these same four argv elements as Codex exec `prefix_rule`, preserve exact native path spelling, and quote command arguments for the active shell. `--out` and `--checkout` may follow that prefix; never put the numeric input or `--out` path in the reusable prefix. Do not launch a numeric-target collector command while claiming canonical-URL preapproval. Do not wrap this command in `rtk`; this exact command boundary takes precedence over generic RTK routing. Do not include a timestamp, report directory, or remaining options in the prefix. Never append a second `--target`, use abbreviated options, wrap the command in another executable, or request a bare interpreter, bare `gh`, collector-wide, or wildcard approval.
...
The brief does not open an approval question. Apply [User Questions](codex-user-questions.md) for missing workflow consent; with existing authorization, proceed directly to the required runtime permission mechanism. Never append a standalone prose confirmation while an eligible native control exists.
