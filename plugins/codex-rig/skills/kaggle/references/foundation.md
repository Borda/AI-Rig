...
Start with `# %% [markdown]` cell: grounded competition title, 2–3 sentences on selected scope/approach, competition URL when known. In inference-only mode, name grounded checkpoint path or Kaggle input dataset.
...
- Import `tqdm` with `from tqdm.auto import tqdm`; use for every visible notebook progress bar (data scans, training-adjacent loops, inference). Never use Rich progress bars.
- Always import `torch` for neural inference/training notebooks.
- Suppress only specific noisy warning category; never blanket-ignore exceptions.
