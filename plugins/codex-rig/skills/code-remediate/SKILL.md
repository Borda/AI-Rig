...
- `matching-review-incomplete:<run-directory>` means an identified review run has no result or candidate yet; it may contain only a recorded PR target before collection, collected `pr.json`, or retained notes. First explain in plain English that the review did not complete; then state `Review handoff blocked`, link that retained run, and name exact failed checkpoint with evidence-backed next action. Return to producer completion checkpoint and perform permitted diagnosis yourself; do not claim no review was performed, consume notes as validated result, select older verdict, or switch to online-only intake. This applies across sessions as well as within one session. A newer candidate or malformed result with damaged PR identity also blocks stale assessed fallback.
...
- Failed required checks, a failed/timeout result, incomplete required closure, or unproven ownership/destination: use `blocked`; state why changes remain unstaged, the next owner/action, the actual gate/closure/ownership evidence, and the controlling rule in this section. Do not ask for commit authorization while blocked. An explicit local commit request may use the environment-only exception below; do not repeat a prior refusal without reassessing it.
- Environment-only exception: when the user explicitly requests a local commit, every remediation-owned code/doc finding and canonical gate is closed, and the only selected unresolved items are environment-owned verification obligations, keep the result failed and those items unresolved but permit `pending` or `committed`. Record the exact user request and source scope under `## Explicit Commit Request` in `commit-plan.md`, and each unrun or failing environment check, evidence, owner, and next action under `## Remaining Verification`. Keep the environment blocker in `unresolved.txt`, final handoff, and the commit message's `Residual limits:`. This exception does not cover a failing canonical gate, source regression, local/process/external-owner blocker, critical finding, unverified ownership, or destination ambiguity. If any precondition is absent, cite the specific rule here and evidence path when declining the commit.
...
10. Result artifact missing => fail. A new remediation handoff omits `commit_disposition`, silently skips step 12, or claims commit readiness with failed canonical verification or a failed result outside the explicit environment-only exception => fail: `remediation-commit-disposition-missing`, `remediation-commit-verification-blocked`, or `remediation-commit-result-blocked`.
