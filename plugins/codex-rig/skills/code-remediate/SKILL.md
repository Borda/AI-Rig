...
For the recommended applicable action, after showing its concrete scope and effects, ask only the matching question: `Authorize finishing this existing merge and the described local commit? (Approve / Deny)` when a commit is needed, `Authorize resolving these existing conflicts without committing? (Approve / Deny)` when no commit is proposed, or `Authorize aborting this existing merge with the stated preservation plan? (Approve / Deny)`. An **Approve** authorizes only the described action; a **Deny** selects **Defer**, preserves current state, and permits only the stated unaffected work. Do not ask all questions in succession or reprompt an action already authorized.
...
After all selected implementation, closure checks, shared quality gates, and result validation finish, write `<run-directory>/commit-plan.md`. It lists only resolved remediation-owned tracked paths and maps each path to its selected finding and work-bucket topic. Do not write plan or ask when no remediation-owned tracked change exists. Otherwise, show complete compact plan, ownership exclusions, and verification limits before asking exactly once through User Questions. Prefer sync only if permitted for authorization and able to represent all four feasible modes; otherwise use async or plain chat without hiding a mode behind Other. The following are canonical choices, not prose to repeat beside a native control:
