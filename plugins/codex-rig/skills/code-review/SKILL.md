...
- State `PR Review Availability: unavailable`, then immediately state `Reason:` with the classified failure before any generic availability, verification, confidence, or next-step text; also state `Source findings: not assessed` and `Merge decision: not made`.
...
- Make the next sentence `Reason: <specific cause>.` before any generic workflow status. For an overlapping dirty worktree, name the exact paths from `worktree-preflight.json` and say checkout would overwrite them; otherwise name the classified failure from `pr-error.txt`.
