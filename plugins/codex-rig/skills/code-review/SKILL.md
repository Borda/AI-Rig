...
- Name classified failure and `<run-directory>/pr-error.txt`, then stop. For `dirty-tracked-worktree-overlap-before-pr-checkout`, explain that checkout would overwrite local changes and link `<run-directory>/worktree-preflight.json` for exact `overlapping_paths`; do not insert protected path lists into the bound summary. Follow the final-handoff redaction contract; never summarize this as generic collector failure.
...
- Start with artifact-bound plain-English explanation, then `PR Review Availability: unavailable` and `Reason: <specific cause>.` before verification details. For overlapping dirty worktree, say checkout would overwrite local changes and link `worktree-preflight.json` for affected paths; preserve final-handoff path redaction. Otherwise name classified failure from `pr-error.txt`. Include available safe command/checkout diagnostics and mark unknown causes explicitly.
