...
Workflow helpers expose their current options through packaged `--help`. Skills batch help checks for the helpers already needed on the selected path and return a compact option index while retaining full output for ambiguous flags. Branch-specific helpers are inspected when that branch is chosen. Each check keeps its own exit status; a failed check blocks the affected helper invocation.
...
> Current source version: `0.25.0`. Codex Rig is peer product to foundry, oss, develop, research, and codemap-py—not copy of repository's `.codex/` configuration.

Adversarial-loop updates show the cumulative convergence table once after each newly validated challenge, not while a review is pending or before any round is validated; new final Results rows show carried + newly found counts and weighted score in every numeric cell. New assessed review candidates must include reviewer attribution and the aggregate summary before promotion, while historical report intake stays compatible. Non-PR assessed candidates additionally need a `Review Snapshot` table whose Scope is the normalized review scope and whose Revision binds retained diff bytes; the collector does not structurally retain target or commit identity.

Current Code Review results use schema 3 for assessed, unavailable, and closed outcomes so the writer and final completion gate agree. Schema-2 reports and schema-3 reports without retained proof remain readable with `find-review-report.py --archive-result` as unverified data; only canonical current schema-3 reports can certify completion or feed remediation.
...
> Current limits at a glance: named-agent shim installation remains platform-blocked; this checkout defines a project-local opt-in `github-read` profile, while other projects need explicit installation of the Codex-home profile; other networked paths retain their runtime approval requirements; Codex CLI, Python 3.10+, and optional `gh`/Kaggle authentication are needed for their respective paths; shim mutation is unsupported on Windows and network/distributed filesystems.
...
- **Scoped network access:** shell networking stays blocked by default outside an active profile. This checkout defines a project-local opt-in `github-read` profile; explicit setup or sync installs the Codex-home profile for other projects without selecting it. Both route GitHub-domain requests through the proxy when selected; host restrictions still apply and HTTP methods are not policy-enforced. Kaggle, marketplace refresh, and paid live calibration retain their runtime approval paths.
...
- Python 3.10+
- On Python 3.10, `tomli` in the same interpreter used for profile setup or `sync_codex.py`; Python 3.11+ uses `tomllib`
...
Codex Rig never enables persistent unrestricted workspace network access. This checkout defines `github-read` in project `.codex/config.toml` without selecting it; explicit setup or sync separately installs the managed Codex-home profile for other projects without changing default permissions. Direct plugin installation alone does not install that managed profile. Start a fresh opted-in session with `codex -c 'default_permissions="github-read"'` when needed. Both extend `:workspace` with GitHub-domain traffic through the network proxy and `.git` write access under workspace roots for every command in the selected session; stricter host restrictions remain in force, and neither enforces HTTP methods or executable identity. Without an active profile, GitHub operations require runtime approval for the complete owning command. Other networked operations retain that approval mechanism; approving only `gh`, `kaggle`, or another nested executable is insufficient when a Python helper owns its subprocesses and HTTPS traffic.

Approval and denial behavior: brief names operation's purpose, capability and effects, target, owning command, and denial outcome. If approval is denied, current tool call stops and assistant turn may end; external command is not run, and Codex Rig does not issue equivalent reprompt or silently broaden fallback. To continue, send new message. Audited GitHub reads and the collector's safe local PR checkout use an active opted-in `github-read` profile without a separate runtime request; otherwise the complete owning command needs runtime approval. Destructive checkout and lifecycle mutation require separate authorization.
...
Use `$codex-rig:challenge-resolve` with a bounded task, selected source, and acceptance criteria. Before fixes, inventory requested paths, material consumers and interactions, and complete an initial challenge across admitted parts; unreviewed parts remain explicit coverage gaps. For each current-source round, a fresh independent challenger returns structured findings; before scoring, the parent checks each failed invariant against every earlier validated signature, assigns severity separately, and dates first-reported findings as preexisting, introduced by remediation, scope-added, or unknown from frozen source evidence. The parent records stable signatures in `loop-ledger.json`, shows the validated weighted table before fixes, and records schema-2 actions in `loop-actions.json` with invariant, original case, consumer, sibling route, and source-path evidence. Each feasible solver receives that closure plan. The parent inspects actual changed source and checks the original and sibling behavior through the consumer, then asks a fresh challenger with the prior findings and complete required source to assess every signature. A solver's report remains pending until that independent assessment. On a repeated scope, prior open weighted findings must fall by at least half for the loop to continue; the existing total-score rule also stops when new discoveries keep the total above half. A changed-scope chunk retains the parent stop and marks prior findings `unassessed` when their validating source is missing; its local baseline cannot reset parent history or prove resolution. The same cycle applies to each large-scope chunk and material interaction, with at most three reviews per run. Missing provenance, a stale snapshot, a failed ledger/action gate, or an unexpected reviewer response stops the affected route with findings visible, one recommended remediation action, alternatives, and a user decision before another affected dispatch. The fresh challenger receives frozen source, criteria, and original counterexamples, never resolver handoffs or implementation explanations. Current challenge results use the same strict artifact checks under both filenames; historical reviewer outputs can be scored as archives but cannot become current challenge rounds by relabeling. Existing review, remediation and implementation workflows retain their own completion gates.

Before the first challenge, the parent maps each workflow branch from entrypoint through producer and ordinary consumer, with a sibling path and acceptance test. Unchanged dependencies join supporting source. A reviewer-declared unassessed route blocks the discovery baseline; a later finding present in older frozen source triggers a coverage-map correction before another fix round. Tests must exercise the real producer output instead of manually rewriting it for the consumer.

Current single-run challenge evidence uses schema 2 with an origin binding. A declared continuation pins its caller ledger and puts every earlier final-round signature, tier, structural flag, and counterexample into the frozen reviewer request; the current reviewer must assess each one. A clean reviewer response declares inspected and uninspected source paths and its limits in structured form. The validator checks that declaration against the frozen source inventory; the declaration cannot prove line-by-line inspection or detect an undeclared earlier run.

After each newly completed validated challenge, the skill invokes `shared/adversarial_loop.py --progress` and shows its cumulative stderr table before any next fix, review or stop. Its columns are `Iteration | Critical | High | Medium | Low | Nits | Weighted score`; each numeric cell is `old + new`, security and critical combine only for display, and score weights remain `20/10/6/4/2/1`. Pending or empty rounds print no progress table; unrelated updates do not repeat it. New final `Results` rows keep security and critical separate while showing `old + new` for each severity and weighted score; `0 + 0` remains visible. Old means a currently open signature seen in any earlier completed round. Unreviewed runs show `N/A` rather than zero; historical result formats remain readable. Table delivery timing is an agent instruction, not a machine-verified chat count. Reviewer responses target at most 1,500 output tokens; this is a hard cap only when the runtime exposes and applies one, never a reason to drop findings.
...
Before a paid isolated Codex challenger turn, `skills/challenge-resolve/validate_evidence.py --preflight-plan` checks the frozen context against current canonical source, diff, declared request, and complete supporting source. Current schema-2 single-run evidence also check declared continuation origin and prior findings before dispatch. Every material section must end exactly at its next label or the context end; evidence-declared supporting paths each need a captured record. Clean final validation checks the retained scoped patch against current Git for both current reviewer routes. Source snapshots and complete-file chunk inventories retain staged deletions as missing records. This no-model check and the separate host check must pass before dispatch; a failed response remains advisory and its frozen evidence stays unchanged.

The loop reuses Code Review's native inspection or isolated Codex reviewer evidence validation, binds the implementation owner to the active host session, and requires every returned finding inside one structured reviewer response. Native specialist manifests use schema 5; isolated Codex reviewer manifests use schema 4. It binds those responses to supplied source contents and recaptures actual scoped source before acceptance. A changed file, staged state, or newly added source invalidates stale clean evidence; a stopped loop must fail its review gate. Older unsupported or substituted reviewer evidence cannot certify independent coverage. These checks preserve each route's provenance limits; they do not prove that a review found every defect.

For a large requested source scope, challenge-resolve first checks native instruction-bounded review instead of applying the optional isolated Codex reviewer route's 2 MiB limit to every route. `skills/challenge-resolve/chunk_diff.py` can split any requested changed or existing source scope into stable whole-file chunks using measured source JSON plus patch bytes and the coordinator's exact `goal`, `specification`, and `done_when`. New schema-1 plans declare first-run or continuation origin; continuation requires `--caller-run`, its exact `loop-ledger.json`, and prior coverage. Continuation planning takes the previously authenticated ledger and a source/reviewer assignment for every prior signature, pins the ledger bytes, and appends each original signature, tier, structural flag, and evidence to the reviewer request. The clean checker requires each assigned current run to receive its declared source and independently mark that exact signature with its original tier and structural flag `verified-fixed` or `rejected`; a new chunk baseline cannot hide old debt. Stopped summaries list unresolved prior signatures; unsupported chunk-plan versions cannot validate clean or stopped results. The parent runs normal validated challenge-resolve loops sequentially per chunk, replans after edits, and checks that every changed file has one current clean child result with matching criteria. Every chunk pair, including a proposed no-interaction decision, receives an independent review of the complete current files in both chunks; declared material relationships among three or more chunks receive a review of the complete member chunks too. A run may assess multiple decisions when its source contains their exact union and its bound request preserves the coordinator criteria plus every assigned rationale. New chunked coordinators store the checker's child-backed summary, and the validated final handoff lists each child's result path, score series, and decision beneath the `chunk-only` Results row. Complete checked child and interaction coverage lets the coordinator pass its review gate with zero findings, without an aggregate score or whole-scope reviewer receipt; partial coverage stops with a failed review gate. Candidate and promoted results rerun the coverage check and bind `metadata.challenge_origin` to the schema-1 manifest declaration. Current candidate and canonical coordinator completion requires schema 1; a falsely declared first run or undeclared caller history cannot be proven from local artifacts. The parent examines possible undeclared groups and the stated review scope before claiming overall clean. Oversized individual files, absent authenticated prior evidence, incomplete declared dependencies, and plausible unassessed interactions remain explicit stops; chunking does not turn partial review into full-scope evidence.

When chunks follow earlier validated reviews, the coordinator retains a chronological finding-history table across the original and child runs. Each row names its run and local round; old/new cells compare assessable underlying defects with earlier validated rows, while the child's own progress table keeps its local numbering. Prior findings whose complete validating source was omitted remain unassessed in coordinator history rather than reclassified as open or new. Different scopes do not form one convergence series or a combined score.

A stopped coordinator with validated partial child work uses a separate partial-results check that preserves each completed child's score and names pending chunks and interactions. The ordinary complete-results check still requires clean child and interaction coverage; neither partial route yields an overall clean verdict.
...
| `challenge-resolve` | Independently review and fix a bounded task through convergence rounds, retaining evidence and explicit stop reasons.                                                                        |
...
Every workflow defines input contract, fail-fast rules, required gates, artifact shape, and confidence output. `shared/quality-gates.md` owns compact outcome-coupled final-chat frame—Outcome, Results, Verification, Remaining, Recommendations / next steps, Confidence, Artifact—while `shared/final-handoff-contract.md` makes that structure executable after gates: schema-v2 general results and schema-v3 current Code Review results bind validated `final-handoff.json`, rendered `final.md`, and digest record before promotion, and workflow emits `final.md` verbatim. Code Review producer completion accepts only bound schema-v3 results, including unavailable and closed dispositions; schema-2 results and schema-3 results without retained proof remain readable as unverified archives, outside completion and remediation intake. Rendered section and table labels use portable Markdown bold text instead of headings or terminal color escapes. Each skill keeps its exact outcome vocabulary, result table, and terminal exceptions; next steps reference result rows instead of repeating them, and artifacts supplement rather than replace readable result. Historical schema-v1 artifacts remain readable. `agent-shims` stays explicit exception because it has no canonical run/result artifact, and current host still has no post-send transcript hook to prove chat transport bytes. `shared/native-skill-contract.md` likewise owns generic shell-network approval and denial behavior while each networked skill retains its five concrete operation values and recovery exceptions. Workflow instructions live in `skills/<name>/SKILL.md`; shared executable contracts live in `shared/`.
...
| `sw-engineer`        | Sol             | Core implementation, APIs, types, and reproducible Python/ML code.               |
| `qa-specialist`      | Sol             | Regression proof, edge cases, test design, and executable acceptance.            |
| `squeezer`           | Sol             | Profile-first performance, throughput, memory, and synchronization analysis.     |
...
| `data-steward`       | Sol             | Dataset integrity, leakage, splits, augmentation, and reproducibility.           |
...
| `curator`            | Luna            | Configuration quality, duplication, drift, stale references, and weak gates.     |
| `challenger`         | Sol             | Adversarial stress tests for plans, risky changes, and no-finding conclusions.   |
| `scientist`          | Sol             | Papers, hypotheses, experimental methods, metrics, and ablations.                |
...
App Server accepts documented planning and warning events with bounded validation. Unknown events still stop that wave; rejection diagnostics classify a documented notification using a fixed public-schema label, or `unrecognized`, without retaining arbitrary method text or payloads. A repeated protocol failure stops retries of that launcher, not accessible source review. Use available instruction-bounded reviewer or disclosed parent-serial fallback without redundant approval; ask for decision only when explicitly required independence remains unavailable. Historical rejected events cannot be reconstructed from payload-free evidence.

Review continuation: verified source → available text-only reviewers → parent inspection if needed → findings with honest coverage. Failed launchers stay stopped; accepted evidence never includes rejected outputs.
...
- Luna owns bounded coordination, documentation, CI/CD, web evidence, OSS, linting, and curation support.
- Sol owns the normal parent, implementation, runtime behavior, tests, data/ML, performance, challenge, and executable verification. Routine parent/build/test/performance use medium; deeper review, data/research method, and challenge use high.
- Sol architecture and security roles remain explicit-selection, read-only advisory passes. Astra has no standing route.
...
The delegation lead returns one handover. Executable acceptance, runtime/API changes, release-blocking decisions, and security/architecture conclusions remain parent- or appropriate Sol-owned. Narrow work stays in parent when handoff cost would exceed its value.
...
Review completion is executable: `shared/find-review-report.py --complete-run <run-directory>` reruns both artifact validators against a promoted schema-v3 result, checks that PR lookup selects that exact result, and emits only digest-bound final text. The optional isolated reviewer route revalidates retained JSON against its frozen source and diff when accepting current evidence, even if recorded output hashes match. A notes-only run is `matching-review-incomplete`, not missing evidence; newer incomplete or malformed reviews block older assessed fallback. A later collection failure cannot clear intervening incomplete or unpromoted review. Failures lead with a plain-English cause and continuation, then state “Review handoff blocked,” preserve preliminary evidence, and never masquerade as completed review or silently switch to online-only remediation. Code Review first supports instruction-bounded native inspection route whose reviewers receive full role card first, then scope inventory and relevant source/diff/evidence as untrusted input, and return text only; prohibited tool use is detected and rejected, not prevented by sandbox claim. Schema-five inspection also accepts call-ID-bound task-path receipts when legacy activity events are absent, requiring unique runtime child lineage and creation within the recorded spawn interval; all context, model, tool-use and terminal-output checks remain mandatory. Missing or ambiguous session logs fail closed. Strict portable launcher admission and post-run validation remain required only for optional portable route and other consumers.

### Evidence collection, review closure, and remediation boundaries

Assessed review headers add a compact `Reviewers` row, such as `Software engineer (3), QA specialist (2), Doc scribe (1).` The legend sits immediately below: 1 = Approve · 2 = Minor changes · 3 = Changes required · 4 = Insufficient evidence · 5 = Block / Reject. Each rating covers that participant's scope; skipped roles are omitted, and ratings are never averaged into the final verdict. Findings add an `Author` column preserving all contributing roles after deduplication. Aggregate prose summaries, existing header fields, verdicts, confidence and evidence remain; historical reports stay readable as unverified archives. Current schema-3 results retain exact reviewer-rating binding for completion and remediation.

GitHub reads use the `github-read` Codex permission profile when selected for a fresh session with `codex -c 'default_permissions="github-read"'`. This checkout defines a project-local profile; explicit setup or repository sync installs the separate managed Codex-home profile for other projects. Adding the plugin alone cannot install that managed profile. Neither profile selects itself by default. Both extend `:workspace`, route requests to GitHub domains through the network proxy, and permit `.git` writes under workspace roots for all commands in the selected session. Setup refuses root `sandbox_mode` or `sandbox_mode` in any named `[profiles.<name>]` entry in the selected Codex home's config because those settings can override `default_permissions`; it also refuses to newly enable the global proxy when an existing named permission profile has `network.enabled = true`. Removal refuses to turn that proxy off while any remaining named permission profile has network enabled. Other loaded configuration layers can still override the opt-in selection; setup validates only the selected Codex home's config. Host restrictions still apply and may block access. The profiles control network destinations, not HTTP methods or executable identity; workflow helpers retain their own operation checks and evidence requirements. Without an active profile, the owning helper needs runtime approval for network access.
...
- **Network boundary:** The profile allows network access to GitHub domains through the proxy for the owning helper and its nested Git/GitHub requests. Destination access does not authorize remote mutations in workflow scope, and host restrictions remain controlling. The profile cannot enforce HTTP methods; helper-side validation and workflow rules govern which operations are used.
...
- **Retry and denial:** When the host blocks a required network operation, follow the host's approval and denial controls. Do not treat profile installation or a prior successful request as an override of a current host restriction; a denial stops the operation.
...
- **Checkout and ancestry:** GitHub CLI remains primary for metadata. In review mode, the collector fetches and verifies the exact PR head, then creates a detached worktree at that immutable commit; it leaves the invoking branch, index, and tracked files untouched even when that worktree is dirty. Source inspection and project gates run from the recorded review worktree, with artifacts retained in the source repository; a failed or dirty generated worktree is retained for diagnosis rather than removed automatically. Remediation mode remains distinct: it always tries `gh pr checkout <canonical PR URL>`, even at matching HEAD, and requires an attached branch; after failure, only a verified same-repository direct checkout of the actual PR branch is allowed. Fork PRs use the bounded adversarial recovery route and return to successful attached `gh` checkout. No remediation exact-commit fallback, generated branch, or manual tracking repair is allowed. The collector fetches target and PR tips without persistent ref destinations on the normal path and captures verified commit IDs; the same-repository fallback may perform a guarded local update of an explicitly selected remote-tracking ref from the already fetched, verified PR head, preserving divergent or concurrently changed refs. Both tips must be fresh before conflict resolution; use captured immutable IDs, never a later `FETCH_HEAD`. Target advancement is integration context, not PR finding or merge blocker; genuine divergence remains collection failure.
...
- **Working-tree identity:** Matching HEAD is necessary but insufficient. Review source verification checks the newly created detached worktree for exact SHA and clean source state; dirtiness in the invoking worktree is preserved and cannot be confused with reviewed PR source. Remediation retains its before/after checkout overlap preflight. Review passes the recorded checkout path as `run_gates.py --worktree` with `--expected-head`; the runner executes gates there and binds that path in before/after source receipts, so a different checkout at the same SHA cannot stand in for the reviewed worktree. New PR review gates require the verified base...head range for `git diff --check`, not the clean worktree's empty diff. For Python tests, the import-bound pytest mode records actual post-test module origins from the selected interpreter and accepts only tracked files in the review worktree; an editable environment still targeting the invoking checkout fails. Free-form or non-Python tests cannot claim source-bound `tests:pass` in the new review artifact; unsupported import patterns remain inconclusive and must be reported.
- **Historical and assessed results:** Historical merged/closed PR evidence uses GitHub's pull ref, exact SHA verification, and an isolated detached review worktree, but merge decisions and code-remediate remain OPEN-only. Every assessed non-approval PR result includes findings/action table only for actual findings or review gates.
...
- **PR number default:** Code Review, Code Remediate, and PR-mode Assess bind a bare PR number to valid GitHub `origin` even when fork remotes exist; without `origin`, they use the sole configured GitHub repository. An explicit canonical PR URL takes precedence and must match a configured remote. Missing or conflicting default identity requires a URL. Direct numeric collector calls remain fail-closed on ambiguous remotes; the skills pass the locally bound URL instead.
- **Fallback eligibility:** Public PR metadata fallback is limited to `github-network`, `github-auth`, `github-rate-limit`, or `command-timeout` failures and requires a canonical URL matching a configured GitHub remote, or a direct numeric collector target with one distinct configured GitHub repository identity. Ambiguous or unsafe direct numeric targets, permission, not-found, and unclassified failures fail closed. GitHub GraphQL object-resolution failures remain not-found errors instead of activating network fallback.
- **Fallback evidence:** The HTTPS client uses Python's default CA store and recovers an available system CA bundle only when that store is empty. The fallback normalizes limited metadata and may verify `refs/pull/<number>/head` in an isolated detached review worktree, derives local diff, and records unavailable evidence in `online-review-summary.json`; it cannot establish private PR evidence or alone satisfy remediation's attached-branch checkout contract. Same-repository remediation still needs its verified original-branch route and all degraded-evidence gates. Fork remediation recovery uses the shared adversarial procedure with at most three rounds including `W_0`; no new loop helper or hand-coded recovery framework is permitted.
...
- **Compact remediation tables:** Initial selection and durable resolution tables show each contributing source only as `report [<report-file>:<line>]`, `report [<report-json>#<finding-id>]`, or `online [<comment|thread|review-id>]`; online references use stable IDs, not URLs, and grouped pointers use one plain space so terminal output never exposes HTML tags. A source cell is unique pointer, never prose or evidence. The final chat outcome table shows each finding and its resolution once; the linked `action-items.md` ledger retains complete sources, resolution explanations, evidence, and next actions. Exact duplicates retain every compact pointer in source order in the ledger and machine metadata. Resolution, evidence, owner/status, and unresolved next actions remain mandatory.
...
It validates packaged skills, role cards, shared contracts, behavior fixtures, accepted routing evidence, confidence scoring, and known workflow leaks. In a source checkout, calibration compares the shipped behavioral case file's integer `schema_version` with its own `HEAD` version; a new version family starts at 1 and an uncommitted edit may advance only one step. An installed cache reports that version check as skipped because its packaged fixture is immutable; source layout reports a missing `.codex` fixture as skipped. The offline CI harness shadows network and LLM commands, uses isolated home, and writes compact failure artifacts without contacting LLM.

Paid live A/B calibration is separate, explicit, and never implied by offline result.

## 🧾 Approval prompts and commit handoffs

Collection prebriefs supply context, not standalone confirmation questions. For an action that requires separate workflow consent, obtain any missing consent through a permitted native control; existing preapproval skips reconfirmation, and runtime permission stays separate. Allowed GitHub reads need no separate workflow consent. Commit readiness also requires closed selected work and a local `commit-plan.md` artifact.

Every new remediation handoff records a commit disposition before final output: pending choice/execution, blocked, no eligible changes, explicitly declined, or committed. Passing runs with eligible owned changes continue from result validation to the existing all-at-once/topic/per-finding/leave-unstaged choice. An explicit local commit request may proceed with a failed remediation result only when all canonical gates and local findings are closed and the sole selected open obligation is environment-owned verification; the result stays failed, the commit plan records the request and remaining check, and the commit message discloses the limit. Other failed verification or required closure leaves changes unstaged with the controlling rule, evidence, and next owner/action. Earlier matching authorization is reused; pending native questions stay pending across resume. The renderer rejects a missing disposition, and candidate validation enforces the narrow exception. Historical saved reports remain readable; no installation is propagated automatically.
...
New final handoffs use `presentation_version=3`: plain-English explanation first, no empty Results section, one concise line for checks not run, and each recovery action shown once with its row ID. Version-2 handoffs retain their original bytes during validation. Unavailable review explanations bind actual collector failure to retained safe command and checkout evidence; missing cause details stay explicitly unknown. Machine records remain complete, historical rendered bytes stay unchanged, and explicitly requested exact caller output is preserved.
...
Review/remediation reports keep one canonical record per finding, not one source per repeated summary/action/confidence mention. New review records add title, issue, required change, evidence and closure criterion under `finding_records_version=1`, and validator requires that marker on every new schema-v3 assessed candidate under both filenames — there is no bare-record fallback for new writes. Older schema-v1 ID/severity-only records and historical rendering remain readable. Real report/online references stay intact; related report mentions do not inflate source counts, and each comment has one owning item.
...
Then start a fresh Codex session. Direct plugin installation does not install the managed Codex-home `github-read` profile; run explicit setup or repository sync for other projects. This checkout defines its own opt-in profile; select either profile only for a session that needs it. Use manager's authenticated `remove` action to clean prior development shims; new installation remains platform-blocked.

Repository sync never restores legacy `.codex/` tree. When Codex scope is active, root `make sync-codex` installs or updates public Codex plugins, authenticated Codex Rig block, and the opt-in `github-read` permission profile, then projects repository model defaults and personal policy as described above. The root Make target supplies no direct-script flags, so it always projects `model`, `review_model`, and personal-policy block together. The direct script's `--no-codex-global-agents` opt-out remains valid for leaving `AGENTS.md` unchanged; it does not skip profile installation. Start a fresh opted-in session after sync when GitHub access is needed.
...
2. While installed package is still available, run explicit `sync_codex.py clear` action to remove managed plugins, authenticated global-instruction block, and the owned `github-read` profile; it backs up changed configuration and preserves unrelated settings.
...
- Plugin removal does not edit `$CODEX_HOME/AGENTS.md` or clear the sync-installed `github-read` profile; managed state remains until explicitly removed.
- The `github-read` profile extends `:workspace` and allows GitHub-domain network traffic through the proxy; it does not enforce HTTP methods. Host restrictions still apply.
