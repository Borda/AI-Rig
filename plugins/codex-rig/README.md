...
> Current release: `0.22.0`. Codex Rig is peer product to foundry, oss, develop, research, and codemap-py—not copy of repository's `.codex/` configuration.

Adversarial-loop updates, approval questions and pauses always show the cumulative convergence table, using `not-run` / `N/A` when no round is validated; final handoffs retain their canonical Results table. New assessed review candidates must include reviewer attribution and the aggregate summary before promotion, while historical report intake stays compatible. Non-PR assessed candidates additionally need a `Review Snapshot` table that earlier releases did not require.
