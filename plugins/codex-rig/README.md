...
- **Checkout and ancestry:** GitHub CLI remains primary for metadata. The collector fetches target and PR tips without persistent ref destinations, captures verified commit IDs, and uses `git checkout --detach <verified-head-oid>` only when HEAD differs. Rewritten PR history no longer requires forced cache updates; existing branches and tracking configuration remain untouched. Both tips must be fresh before conflict resolution; use the captured immutable target ID, never a later `FETCH_HEAD`. Target advancement is integration context, not PR finding or merge blocker; genuine divergence remains collection failure.
