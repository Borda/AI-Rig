...
> Current release: `0.23.0`. Codex Rig is peer product to foundry, oss, develop, research, and codemap-py—not copy of repository's `.codex/` configuration.

Adversarial-loop updates show the cumulative convergence table once after each newly validated challenge, not while a review is pending or before any round is validated; final handoffs retain their canonical Results table. New assessed review candidates must include reviewer attribution and the aggregate summary before promotion, while historical report intake stays compatible. Non-PR assessed candidates additionally need a `Review Snapshot` table that earlier releases did not require.
...
Use `$codex-rig:challenge-resolve` with a scoped diff and acceptance criteria to run independent review-and-fix rounds. Root guidance keeps mandatory guardrails; `shared/adversarial-loop.md` remains the detailed procedure, also shipped as generated copies by the Claude plugins. Each round challenges and collects findings, reports the old/new table, resolves feasible in-scope findings, escalates unfixable `security`/`critical`/`high` findings, then repeats until clean, three reviews or plateau. Structural and repeated findings remain eligible for authorized fixes; missing authority or recurrence evidence stops only the affected workstream. The skill records scores, stable finding dispositions, parent actions in `loop-actions.json`, retained source snapshots and original reviewer reports; validation rejects inconsistent histories, omitted finding actions and false clean claims for candidate and final results. Older result files remain readable as data but cannot claim current validation without the action contract. Matching finding verdicts combine exact evidence from every reviewer in first-seen order; conflicting severity, structural classification or disposition still blocks acceptance. Existing review, remediation and implementation workflows retain their own completion gates.

After each newly completed validated challenge, the skill invokes `shared/adversarial_loop.py --progress` and shows its cumulative stderr table before any next fix, review or stop. Its columns are `Iteration | Critical | High | Medium | Low | Nits | Weighted score`; each numeric cell is `old + new`, security and critical combine only for display, and score weights remain `20/10/6/4/2/1`. Pending or empty rounds print no progress table; unrelated updates do not repeat it. The final `Results` table remains separate. Table delivery timing is an agent instruction, not a machine-verified chat count. Reviewer responses target at most 1,500 output tokens; this is a hard cap only when the runtime exposes and applies one, never a reason to drop findings.
...
| `challenge-resolve` | Independently review and fix a scoped diff through bounded convergence rounds, retaining evidence and explicit stop reasons.                                                                 |
...
Assessed review headers add a compact `Reviewers` row, such as `Software engineer (3), QA specialist (2), Documentation reviewer (1).` The legend sits immediately below: 1 = Approve · 2 = Minor changes · 3 = Changes required · 4 = Insufficient evidence · 5 = Block / Reject. Each rating covers that participant's scope; parent substitutes are labeled, skipped roles omitted, and ratings never averaged into the final verdict. Findings add an `Author` column preserving all contributing roles after deduplication. Aggregate prose summaries, existing header fields, verdicts, confidence and evidence remain; historical reports stay readable.
...
- **One-flag authorization:** Use `$codex-rig:code-review 123 --approve-gh` or `$codex-rig:code-remediate 123 --approve-gh` (also supported by the unnamespaced skill aliases). The flag records completed user authorization for required PR collection; it cannot suppress a Codex host permission prompt by itself. For a numeric target, the local-only `select-git-remote.py --canonical-pr-url <number> --cwd <repository>` step must resolve exactly one configured GitHub repository before collection. Its canonical URL becomes both collector `--target` and the direct reusable command prefix `actual Python executable + absolute installed collect_pr.py + --target + canonical GitHub PR URL`; `--out`, `--checkout`, and the mode-specific `--checkout-mode` follow that prefix, report timestamps remain outside it, and no `rtk` wrapper is allowed. An ambiguous or absent local identity fails closed instead of guessing a repository; a current-branch request without authoritative PR identity uses one-shot approval. This boundary covers GitHub evidence, fetches, collection artifacts, and safe local checkout effects for that PR. Runtime permission remains separate: only a loaded matching host allow rule without stricter restrictions can avoid its prompt. The flag cannot create rules, override denial, change permissions, or authorize remote mutation. Saved rules need a new match after collector path/version or PR identity changes. Local/report-only scopes reject the flag; remediation still asks which findings to fix. The collector rejects duplicate targets and abbreviated options so later arguments cannot override the approved PR.
