...
Use `$codex-rig:adversarial-loop` with a scoped diff and acceptance criteria to run independent review-and-fix rounds. Root guidance keeps mandatory guardrails; `shared/adversarial-loop.md` is the single detailed procedure, also shipped as generated copies by the Claude plugins. Each round challenges and collects findings, reports the old/new table, resolves feasible in-scope findings, escalates unfixable `security`/`critical`/`high` findings, then repeats until clean, three reviews or plateau. Structural and repeated findings remain eligible for authorized fixes; missing authority or recurrence evidence stops only the affected workstream. The skill records scores, stable finding dispositions, parent actions in `loop-actions.json`, retained source snapshots and original reviewer reports; default validation rejects inconsistent histories, omitted finding actions and false clean claims for candidate and final results. An explicit archive-only validator option keeps old final results inspectable without weakening new promotion. Matching finding verdicts combine exact evidence from every reviewer in first-seen order; conflicting severity, structural classification or disposition still blocks acceptance. Existing review, remediation and implementation workflows retain their own completion gates.
