...
Review, implementation and management apply the same launcher compatibility admission before parallel reads. `auto` falls back to serial with a recorded reason when required child controls are unavailable; explicit parallel-read stops. BROAD/HIGH_RISK review still requires real independent QA/challenger passes, so serial fallback cannot complete it. Shared orchestration applies mandatory-control checks to every other skill before dispatch. Requested role settings, inherited parent settings, supported launcher controls and observed effective controls remain distinct; no plan declaration proves sandbox isolation. A workspace-write launcher without verified child overrides remains unsupported for strict independent review. Diagnose this as a process limitation, not a source defect or failed launch; no live compatibility is claimed by offline tests.
