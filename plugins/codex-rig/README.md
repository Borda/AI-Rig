...
> Current release: `0.16.3`. Codex Rig is peer product to foundry, oss, develop, research, and codemap-py—not copy of repository's `.codex/` configuration.
...
| Skill             | Purpose                                                                                                             |
| ----------------- | ------------------------------------------------------------------------------------------------------------------- |
| `change-analysis` | Inspect issue, PR, module, or problem before implementation; emit ranked findings and gates.                        |
...
- **Approval boundary:** In network-sandboxed runtime, complete collector command—not standalone `gh` preflight—must receive external-network approval because its nested CLI, HTTPS fallback, checkout, and Git fetches inherit collector's execution context.
- **Retry and denial:** If agent mistakenly launched that collector without approval and receives sandbox-shaped `github-network` failure before any user prompt or denial, one unchanged collector retry through approval mechanism is required before failure becomes terminal. A user denial always stops current turn and forbids that retry.
