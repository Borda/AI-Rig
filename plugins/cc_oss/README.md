...
Resolve's caller-context pre-loop groups selected items by canonical module: one live `rdeps` query per module, or none for a fresh cached answer, including an empty caller list. Subsequent structural preparation reuses the file-to-module map only while index identity, file stamp, and requested files match. Cache stamps include the canonical index path; old path-free stamps safely miss instead of reusing another project's answer. Codemap source inspection remains legitimate when implementation details—not the already-answered structural fact—are needed.
