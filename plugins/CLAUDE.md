...
Single-line sentinel read-back: `IFS= read -r VAR < "${TMPDIR:-/tmp}/<name>-${CSID}" 2>/dev/null || VAR=<default>` — never `VAR=$(cat ...)`. Command substitution `$(...)` triggers "Contains expansion" permission prompt in subagents regardless of allow list. Bare `cat "$VAR/foo.md"` (section above) unaffected — no substitution. Full rule + newline caveat: `cc_foundry/rules/claude-config.md` §TMPDIR Sentinel Scoping. Every plugin registers one Bash `PreToolUse` auto-allow hook, `hooks/allow-dispatch.js`, and ships two decision modules behind it (canonical: cc_foundry; all propagated via `propagate_shared.py`). `hooks/sentinel-read-allow.js` matches by shape: the legacy idiom, `$(date -u +FMT)` stamps, and the read-form itself (its first token `IFS=` matches no prefix allow-rule) in read-only compounds. For capture-substitutions no shape can cover (`$(python …)`, `$(git rev-parse …)`, `$(jq …)`), `hooks/blueprint-allow.js` matches by provenance: the normalized command must be an exact digest hit in the plugin's committed `blueprint-manifest.json`. Neither module is registered itself; the dispatcher asks them in rank order — provenance first — emits the first allow, and appends one audit record describing what both said. Full detail in `cc_foundry/permissions-guide.md` §"Contains expansion" gate and §Audit records.
...
- **A literal default beside a `# substitute:` comment is a placeholder in disguise.** `MODE=each  # substitute: each | grouped` matches the manifest unedited, so the default silently wins on every run (measured: a resolve run that selected grouped landed 12 per-item commits). A user-chosen value from a closed set → one fixed block per value (`echo grouped > "$SENTINEL"`), the orchestrator runs exactly the matching one. Free-text values keep a placeholder but the block aborts on it unsubstituted (`case "$X" in *'<'*'>'*|"") echo "! BLOCKED …"; exit 1;; esac`). Never cite an existing `# substitute` block as precedent. <!-- policy-sibling: plugins/CLAUDE.md §Blueprint Blocks (canonical), plugins/cc_foundry/agents/challenger.md, plugins/cc_oss/skills/resolve/SKILL.md (Step 3d, Step 10), plugins/cc_oss/skills/review/SKILL.md (reject gate) -->
...
- **Agent spawns require health monitoring**: every `Agent()` spawn runs in the background — the skill spawns, ends the turn, and resumes on the completion notification. Any skill calling `Agent()` must implement CLAUDE.md §6 (sentinel + single liveness probe + cutoff, never a fixed-interval poll or a no-op call holding the turn open) — reference `_FOUNDRY_SHARED/agent-spawn-protocol.md`, don't reproduce inline. Check C35.
...
- For a manifested file, edit canonical then run `propagate_shared.py --apply`; check mode is the default and takes no flag (`--check` exits 2), enforced in pre-commit and audit Check 14e.
- Not manifested: an optional provider's contract that is worthless without the provider (codemap-py's `claude-skills/_shared/*.md`) — consumers read the active install live; see §Self-Contained `_shared` optional-provider exception.
...
- **Optional-provider contract exception** — the one sanctioned runtime read of another plugin's file. A consumer may read a *provider* plugin's shipped file when all three hold: (a) the consumer's feature is a no-op without that provider (gated on the provider's CLI/presence), so the content has no value in the provider's absence; (b) resolution goes through the active install record — `resolve_shared_path.py <provider> <subdir>`, registry tier first; the resolver's newest-cache tier (which skips `.orphaned_at` dirs) is a degraded fallback reached only when no usable install record exists or the recorded install lacks the subdir, and a wrapper never globs the cache itself; (c) absence degrades to the consumer's own fallback line. Current instance: `resolve_shared_path.py codemap-py claude-skills/_shared` for `codemap-gates.md` / `codemap-context.md`. A frozen manifested copy would drift silently against whatever provider version the user actually runs. Content needed *to survive* a plugin's absence (agent-resolution tables, degraded-mode instructions) never qualifies — the catch-22 rule below stands.
