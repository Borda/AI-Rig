...
Debriefs isolate recent version/project/runtime cohorts. `join_avoidance.py` labels `avoidance_count` and `rate` as `module_overlap_proxy_v3`: verified successful complete answers followed by matching tool targets, including legitimate source reads. This is not confirmed misuse, a guard-failure rate, or measured token savings. New CLI/tool/skill records carry their originating project root; shared log destinations never replace it. Legacy records missing project identity or terminal outcome remain untouched and counted, but cannot support joins. Batch children are logical answers, not extra CLI invocations; failed and unjoinable children are counted separately in JSON and text. Changed eligibility makes earlier metric rates non-comparable. Each overlap also carries a `kind`: `source_read` for verified own-file inspection, `structural_search` for a directory search, or `unknown` when Grep/Glob scope is absent or indeterminate; a `file` scope is `source_read` whichever file it names (a single-file search is inspection), a `directory` scope is `structural_search`. New Grep/Glob records preserve `search_path` and producer-observed `search_scope`; legacy logs remain untouched and unknown cases do not count toward `structural_search_count`; `unknown_count` is reported beside it, overall and per runtime, so zero structural searches can be told from nothing classifiable. This remains a shape proxy, not proof of redundant work. Bash targets are cut at 200 characters, so later flags or paths are unavailable; recursive-looking Bash searches outside own-file inspection stay `unknown`, never presumed directory searches. Hook-triggered `changed_count` is the eligible staged-path delta against the index commit, filtered by scanner exclusions while retaining indexed documentation paths; it is not a measured reparse count. Unstaged/untracked paths are outside that count, and invalid or unreachable revisions leave it unknown. Explicit-root CLI telemetry and Claude marker lookup follow the resolved target project, not the caller directory.
...
- The shared Claude/Codex prompt hook remains silent and skips refresh when scanner exclusions leave no indexable `.py` or `.pyi` sources. Missing-index guidance keeps its bounded package/packaging marker check, so marker-free scripts require a manual `codemap-py index`; an existing index with real source follows the normal status and refresh path.
...
> Codex questions use a [short shared guide](https://github.com/Borda/AI-Rig/blob/main/plugins/codemap-py/shared/codex-user-questions.md), shipped locally by this plugin. The root asks with permitted native controls, meaningful presets and built-in custom input; children hand decisions back to the root. Detailed approval/recovery rules load only when needed. Host restrictions still apply; this does not override a plain-text-only host.
