...
- Exception: single-agent single-question spawns with inherently short output (\<200 tokens)
...
2. Return to orchestrator **ONLY** a compact JSON envelope on the final line — nothing after it:
...
Add task-specific keys (e.g. `"papers":5` for research, `"verdict":"approve"` for review), keep envelope ≤250 bytes. `summary` = one-line description of what found/done — always required.

## RUN_DIR convention

Three tiers — pick by what the dir holds:
...
**Key rule**: `.reports/<skill>/` holds ONLY final consolidated outputs. Intermediate subagent handover files (agent `.md` analysis files) go to `.temp/<skill>/<timestamp>/`, never `.reports/`.
...
4. Spawn **consolidator agent** to read all `<RUN_DIR>/*.md`, write final report
...
- Consolidator type: same domain as lead reviewer (`foundry:sw-engineer` for code review, `foundry:curator` for config audit)
...
`/calibrate`, `/audit` predate this convention — mix intermediate and final in `.reports/<skill>/`. Treat as legacy patterns, not examples to follow. Migration pending.
